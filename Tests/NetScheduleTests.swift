...
final class NetScheduleTests: XCTestCase {
    ...
    func testDstSpringForwardKeepsWallClock() {
        ...
        let before = utcDate(2_026, 3, 7, 12, 0)
        let start = NetSchedule.nextOccurrence(of: denver, after: before)
        XCTAssertEqual(start, utcDate(2_026, 3, 9, 1, 0),
    ...
    func testUtcNetCrossingLocalDayBoundary() {
        ...
        let monday = utcDate(2_026, 9, 7, 12, 0)
        let start = NetSchedule.nextOccurrence(of: utc, after: monday)
        XCTAssertEqual(start, utcDate(2_026, 9, 8, 1, 0))
    }
    ...
    func testLeadRollsToPreviousWeekday() throws {
        ...
        let start = try XCTUnwrap(NetSchedule.nextOccurrence(
            of: early, after: utcDate(2_026, 9, 9, 0, 0)
        ))
    ...
    func testIsLiveWindow() {
        let weekly = net(weekdays: [4], hour: 12, minute: 0, zone: "UTC")
        XCTAssertTrue(NetSchedule.isLive(weekly, at: utcDate(2_026, 9, 9, 12, 30)))
        XCTAssertFalse(NetSchedule.isLive(weekly, at: utcDate(2_026, 9, 9, 13, 30)))
        XCTAssertFalse(NetSchedule.isLive(weekly, at: utcDate(2_026, 9, 9, 11, 59)))
    }
    ...
    private func net(weekdays: [Int], hour: Int, minute: Int,
                     zone: String) -> Net
    {
        Net(name: "Test", talkgroup: 3_100, weekdays: weekdays,
