...
final class ASLDirectory: ObservableObject {
    ...
    nonisolated static func resolve(node: String) async -> (host: String, port: UInt16) {
        ...
        for resolver in resolvers {
            guard let url = URL(string: resolver) else {
                continue
            }
            var request = URLRequest(url: url, timeoutInterval: 6)
            request.setValue("application/dns-json", forHTTPHeaderField: "Accept")
            guard let (data, _) = try? await URLSession.shared.data(for: request),
                  let parsed = parseDoHTXT(data)
            else {
                continue
            }
            return parsed
        }
        return (host: name, port: 4_569)
    }
    ...
    nonisolated static func parseDoHTXT(_ data: Data) -> (host: String, port: UInt16)? {
        ...
        for record in answers {
            ...
            for piece in record.data.split(whereSeparator: { "\" ".contains($0) }) {
                if piece.hasPrefix("IP=") {
                    address = String(piece.dropFirst(3))
                }
                if piece.hasPrefix("PT=") {
                    port = UInt16(piece.dropFirst(3))
                }
            }
            if let address {
                return (host: address, port: port ?? 4_569)
            }
        }
        return nil
    }
